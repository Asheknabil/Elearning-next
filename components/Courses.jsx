...
const Course = ({ course }) => {
  ...
  const badgeColor = course.badge === "New" ? "bg-[#0fb6e3]" : "bg-[#16748e]";
...
export default function Courses() {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const flatCourses = Object.keys(data).flatMap(category =>
          data[category].map(course => ({ ...course, category }))
        );
  ...
  const searchedCourses = useMemo(() => {
    if (!searchQuery) return currentCategoryCourses;
    return currentCategoryCourses.filter(course =>
      course.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [searchQuery, currentCategoryCourses]);

  const displayedCourses = useMemo(() => {
    if (showAll) return searchedCourses;
    return searchedCourses.slice(0, 4);
  }, [showAll, searchedCourses]);

  const hasMore = searchedCourses.length > 4;
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto mb-12 text-center mt-20">
        ...
        <div className="mt-10 flex flex-col md:flex-row items-center md:items-start justify-between gap-4">
          <h1 className="w-full md:w-7/12 text-2xl sm:text-3xl md:text-4xl font-extrabold text-black/80 leading-snug text-center md:text-left">
            {showAll ? "All Courses from All Categories" : `Pick Our Latest Courses in ${activeFilter}`}
          </h1>
          ...
          <div className="w-full md:w-5/12">
            <div className="flex items-center bg-gray-200 rounded-md px-2 py-1">
              ...
              <input
                ...
                className="w-full bg-gray-200 text-gray-700 focus:outline-none py-2 px-2 rounded-md"
                value={searchQuery}
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-6 py-2 rounded-full font-semibold transition-all duration-300 shadow-md ${
                activeFilter === cat
                  ? "border border-2 border-[#0fb6e3] inline-flex items-center gap-2 text-black font-medium px-6 py-3 rounded-full hover:bg-[#0fb6e3]"
          ...
          <div className="flex justify-center mt-12">
            <button
              ...
              className={`px-6 py-2 rounded-full font-semibold text-white transition-all duration-300 shadow-md ${
                showAll ? "bg-[#0a4d5f] shadow-blue-300/50" : "group bg-[#0fb6e3] hover:shadow-lg hover:scale-99"
              }`}
            >
